...
class NetValuePolicy(torch.nn.Module):
    ...
    # The features tensor arguments should be dense tensors and not sparse tensors
    # as the former are way faster
    def forward(self, stm_features_tensor, ntm_features_tensor, legal_moves_idxs1882_tensor):
        ...
        assert legal_moves_idxs1882_tensor.dtype == torch.bool
        ...
        logits[legal_moves_idxs1882_tensor == False] = torch.finfo(logits.dtype).min

        #policy = torch.nn.functional.softmax(logits, dim=1, dtype=logits.dtype)

        return self.hidden_to_out_value(hidden_layer), logits
