...
constexpr u8 getPieceTypeMoving(const u16 move) { return isPromo(move) ? 0 : (move >> 12) & 0b111; }

constexpr u8 getPromotionPieceType(const u16 move) {
    return isPromo(move) ? (move >> 12) & 0b111 : 6;
}

constexpr u16 moveAsU16(const u8 src, const u8 dst, const u8 pieceTypeMoving,
                        const u8 promoPieceType = 6) {
    assert(src < 64);
    assert(dst < 64);
...
constexpr i16 getMoveIdx1882(const u8 src, const u8 dst, const u8 pieceTypeMoving,
                             const u8 promoPieceType) {
    assert(src < 64);
    assert(dst < 64);
    assert(pieceTypeMoving < 6);
    assert(promoPieceType != 0 && promoPieceType != 5 && promoPieceType <= 6);
