    ...
    for (size_t entryIdx = 0; entryIdx < BATCH_SIZE; entryIdx++) {
        ...
        while ((pieceData = dataEntry.popOrientedPiece()).has_value()) {
            ...
            batch.activeFeaturesStm[idx] =
                dataEntry.inCheck() * 768 + static_cast<i16>(pieceColor) * 384 +
                static_cast<i16>(pieceType) * 64 + static_cast<i16>(square ^ stmXor);

            batch.activeFeaturesNtm[idx] =
                dataEntry.inCheck() * 768 + static_cast<i16>(!pieceColor) * 384 +
                static_cast<i16>(pieceType) * 64 + static_cast<i16>(square ^ ntmXor);

            batch.numActiveFeatures++;

            chess::Piece piece = chess::Piece(static_cast<chess::PieceType::underlying>(pieceType),
        ...
        batch.bestMoveIdx1882[entryIdx] =
            getMoveIdx1882(moveSrc(dataEntry.bestMoveOriented()) ^ stmXor,
        ...
        if (dataEntry.kCastleRightOriented()) {
            board.cr_.setCastlingRight(chess::Color::WHITE,
        ...
        for (const auto& move : moves) {
            const i16 moveIdx1882 = getMoveIdx1882(static_cast<u8>(move.from().index()) ^ stmXor,
                                                   static_cast<u8>(move.to().index()) ^ stmXor,
                                                   static_cast<u8>(board.at(move.from()).type()),
                                                   move.typeOf() == chess::Move::PROMOTION
                                                       ? static_cast<u8>(move.promotionType())
                                                       : 6);
