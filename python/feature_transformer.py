...
class FeatureTransformerSliceFunction(autograd.Function):
    ...
    def forward(ctx, feature_indices, weight, bias):
        ...
        output = torch.empty(batch_size, output_size, dtype=torch.float32, device=feature_indices.device, requires_grad=True)
    ...
    def backward(ctx, grad_output):
        ...
        weight_grad = torch.zeros(weight.shape[0], weight.shape[1], dtype=torch.float32, device=feature_indices.device)
        bias_grad = torch.zeros(output_size, dtype=torch.float32, device=feature_indices.device)
...
class FeatureTransformerSlice(nn.Module):
    def __init__(self, num_inputs: int, num_outputs: int):
        ...
        self.weight = nn.Parameter(torch.rand(num_inputs, num_outputs, dtype=torch.float32) * 0.2 - 0.1)
        self.bias   = nn.Parameter(torch.rand(num_outputs, dtype=torch.float32) * 0.2 - 0.1)
